...
def get_known_words(user_id: str) -> List[str]:
    res = supabase.from_("known_words").select("word").eq("user_id", user_id).execute()
...
def add_known_words(user_id: str, words: List[str]) -> int:
    ...
    supabase.from_("known_words").insert(rows).execute()
    return len(rows)

def replace_known_words(user_id: str, words: List[str]) -> None:
    supabase.from_("known_words").delete().eq("user_id", user_id).execute()
    add_known_words(user_id, words)

def delete_known_word(user_id: str, word: str) -> None:
    supabase.from_("known_words").delete().eq("user_id", user_id).eq("word", word).execute()